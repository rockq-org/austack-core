/**
 * Module for initializing the loginRecord api request parameters for /api/loginRecords routes.
 * Export the {@link loginRecord:Parameters~registerLoginRecordParams}
 * function to register the api routes on the passed express router.
 * @module {function} loginRecord:parameters
 * @requires {@link loginRecord:model}
 */
'use strict';

// export the function to register all loginRecord request params
module.exports = registerLoginRecordParams;

/**
 * Attach request parameters to the given router.
 * @param router {express.Router} - The router to attach the parameters to
 */
function registerLoginRecordParams(router) {
	// router.param('id', registerParamName);
	// add params below
}

/*
 * Register a parameter for /api/loginRecords requests.
